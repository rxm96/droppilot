...
const sameChannel = (left: ChannelEntry, right: ChannelEntry): boolean =>
  left.id === right.id &&
...
const mergeChannelList = (prev: ChannelEntry[], next: ChannelEntry[]): ChannelEntry[] => {
...
const sortChannelsByViewers = (channels: ChannelEntry[]): ChannelEntry[] =>
...
const applyLiveDiff = (prev: ChannelEntry[], payload: ChannelLiveDiff): ChannelEntry[] => {
...
export function useChannels({
  ...
  const applyChannelsState = (next: ChannelEntry[]) => {
  ...
  const isFresh = (game: string, now = Date.now()) =>
    fetchedAt !== null &&
    fetchedGame === game &&
    now - fetchedAt < TRACKER_REFRESH_WINDOW_MS &&
    channels.length > 0;
  ...
  }, [allowWatching, demoMode, shouldTrackChannels]);
