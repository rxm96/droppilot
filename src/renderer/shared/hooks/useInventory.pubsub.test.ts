...
describe("applyDropProgressToInventoryItems", () => {
  ...
  it("caps earned minutes at required minutes", () => {
    const items = [makeItem({ requiredMinutes: 10, earnedMinutes: 2, status: "progress" })];
    const payload: UserPubSubEvent = {
      kind: "drop-progress",
      at: Date.now(),
      topic: "user-drop-events.1",
      messageType: "drop-progress",
      dropId: "drop-1",
      currentProgressMin: 50,
      requiredProgressMin: 10,
    };
    const patched = applyDropProgressToInventoryItems(items, payload);
    expect(patched.changed).toBe(true);
    expect(patched.items[0].earnedMinutes).toBe(10);
    expect(patched.deltaMinutes).toBe(8);
  });
});
...
describe("applyDropClaimToInventoryItems", () => {
  ...
  it("matches drop claims by dropInstanceId when dropId is missing", () => {
    const items = [makeItem({ earnedMinutes: 5, status: "progress", dropInstanceId: "inst-42" })];
    const payload: UserPubSubEvent = {
      kind: "drop-claim",
      at: Date.now(),
      topic: "user-drop-events.1",
      messageType: "drop-claim",
      dropInstanceId: "inst-42",
    };
    const patched = applyDropClaimToInventoryItems(items, payload);
    expect(patched.changed).toBe(true);
    expect(patched.items[0].status).toBe("claimed");
  ...
  it("returns unchanged when drop is already claimed with full progress", () => {
    const items = [makeItem({ earnedMinutes: 60, status: "claimed", dropInstanceId: "inst-1" })];
    const payload: UserPubSubEvent = {
      kind: "drop-claim",
      at: Date.now(),
      topic: "user-drop-events.1",
